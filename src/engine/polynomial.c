...
#include "solver/params.h"
#include "solver/kem.h"
...
void wrap_hybrid_sk(const uint8_t *hybrid_sk, const uint8_t *master_key,
    ...
    if (use_krakken) {
        aes256gcm_encrypt(hybrid_sk, HYBRID_SK_LEN, master_key, nonce_out, WRAP_NONCE_LEN,
                          (const uint8_t *)"WRAP", 4, ciphertext_out, ciphertext_out + HYBRID_SK_LEN);
    } else {
...
int unwrap_hybrid_sk(uint8_t *hybrid_sk, const uint8_t *master_key,
    ...
    if (use_krakken) {
        return aes256gcm_decrypt(ciphertext, HYBRID_SK_LEN, ciphertext + HYBRID_SK_LEN,
                                 master_key, nonce, WRAP_NONCE_LEN,
                                 (const uint8_t *)"WRAP", 4, hybrid_sk) >= 0 ? 0 : -1;
    } else {
